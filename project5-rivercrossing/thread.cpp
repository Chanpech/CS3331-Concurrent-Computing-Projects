...
Cannibal::Cannibal(int c, int climit)
{
    ...
    cannibalsArray = new int[climit];

    for(int i = 1 ; i <= cannibalsLimit; i++){
        cannibalsArray[i] = 0;
    }
}
...
Missionary::Missionary(int m, int mlimit)
{
    ...
    missionariesArray = new int[mlimit];
    
    for(int i = 1 ; i <= missionariesLimit; i++){
...
void Missionary::ThreadFunc()
{
    ...
    {
        Delay();                                                  // take a rest
        boat.MissionaryArrives(missionaryNum, missionariesLimit, missionariesArray); // register to cross the river
        // other stuffs
